...
창 없이 압축만 하는 콘솔 명령입니다(토탈 커맨더 등의 외부 호출용). 옵션은 Bandizip 과 같습니다.
...
| --- | --- |
| `-l:0..9` | 압축 레벨 |
| `-fmt:zip\|7z\|tar` | 형식(생략하면 출력 확장자) |
| `-v:700M` | 분할 크기(K/M/G) |
| `-p:암호` | 암호 |
| `-t:N` | 스레드 수(7z) |
| `-aou` | 같은 이름이 있으면 `이름 (2)` 로 |
| `-aos` | 같은 이름이 있으면 건너뜀 |
| `-testdst` | 압축 후 무결성 검사 |
| `-delsrc` | 검사를 통과하면 원본 삭제 |
| `-date` | 파일 이름의 `%Y %y %m %d %H %M %S` 를 현재 시각으로 |

종료 코드는 0(성공) / 1(경고: 빠진 항목, 검사 실패, 건너뜀) / 2(오류) 입니다. 전체 옵션은 `ZipMania.exe --help` 로 봅니다.
